...
@export var box_sprite : AnimatedSprite2D
...
func advance():
	#TODO: advance definition
	#this function pauses the command loop until the advanced signal fires (player clicks advance key.)
	await advanced
...
func next_char():
	...
	if delta_index >= delta_char:
		...
		text_queue.erase(0, 1)
	
	
#command definitions
func open_text_box(p : Dictionary):
	#TODO: opent_text_box() definition
	set_visible(true)
	main_label.text = ""

func close_text_box(p : Dictionary):
...
func wait_for_advance(p : Dictionary) -> void:
	#TODO: wait_for_advancce definition
	pass
